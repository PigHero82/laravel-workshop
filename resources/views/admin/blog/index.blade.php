            ...
            @foreach ($data as $item)
              <tr>
                ...
                <td class="align-middle">
                  <div class="btn-group">
                    <a href="{{ route('admin.blog.show', $item->id) }}" class="btn btn-sm btn-info text-white"><i data-feather="eye" size="14"></i></a>
                    <a href="{{ route('admin.blog.edit', $item->id) }}" class="btn btn-sm btn-warning text-white"><i data-feather="edit-2"></i></a>
                    <button type="button" class="btn btn-sm btn-danger" onclick="deleteData({{ $item->id }})"><i data-feather="trash-2"></i></button>

                    <form id="form-{{ $item->id }}" method="POST" action="{{ route('admin.blog.destroy', $item->id) }}">
                      @csrf
                      @method('DELETE')
                    </form>
                  </div>
                ...
                <td class="align-middle">
                  <img class="img-thumbnail" src="{{ Storage::url($item->image) }}" alt="{{ $item->name }}" />
                </td>
